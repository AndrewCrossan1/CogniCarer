    ...
    return (
        <View className={"flex-1 dark:bg-neutral-800"} style={{zIndex: 0, height: height}}>
            ...
            <View className={"android:mt-safe ios:mt-2"} style={{zIndex: 1000}}>
                ...
                {hasErrors ? <Alert message={"Please fill in all required fields"} visible={localVisible} onPress={() => {setLocalVisible(false)}} type={"error"}/> : null}
                {/* Alert for success */}
                {successVisible ? <Alert message={"Account information updated!"} visible={successVisible} onPress={() => {setSuccessVisible(false)}} type={"success"}/> : null}
            </View>
